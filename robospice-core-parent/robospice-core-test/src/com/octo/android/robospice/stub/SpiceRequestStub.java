...
/**
 * A {@link SpiceRequest} that is stubbed to provide testable state.
 * 
 * Typically, when testing parts of RoboSpice, we will create a request and send it to the engine. As the engine is
 * asynchronous the request is processed in a different thread than the testing thread. This stub allows to wait for
 * request to be completed to make assertions in the test. It will also provide testable states (knowing if methods have
 * been called or not).
 * 
 * @author sni
 * 
...
public abstract class SpiceRequestStub< T > extends SpiceRequest< T > {
    ...
    /**
     * @return a boolean to indicate whether or not the {@link #loadDataFromNetwork()} method has been called.
     */
