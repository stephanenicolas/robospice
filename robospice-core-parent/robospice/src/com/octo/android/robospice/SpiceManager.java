...
public class SpiceManager implements Runnable {
        ...
        /**
         * The list of all requests that have not yet been passed to the
         * service. All iterations must be synchronized.
         */
        ...
        /**
         * The list of all requests that have already been passed to the
         * service. All iterations must be synchronized.
         */
        ...
        private final Condition conditionServiceBound = lockAcquireService
                        .newCondition();
        /** A monitor to ensure service is unbound. */
        private final Condition conditionServiceUnbound = lockAcquireService
                        .newCondition();
        ...
        /**
         * Creates a {@link SpiceManager}. Typically this occurs in the
         * construction of an Activity or Fragment. This method will check if
         * the service to bind to has been properly declared in AndroidManifest.
         ...
         * {@link SpiceService}.
         * @param context
         *                a context that will be used to bind to the service.
         *                Typically, the Activity or Fragment that needs to
        ...
        public void run() {
                ...
                try {
                        ...
                        Ln.d(e,
                                        "Interrupted while waiting for acquiring service.");
                } finally {
        ...
        /**
         * Stops the {@link SpiceManager}. It will unbind from
         * {@link SpiceService}. All request listeners that had been registered
         * to listen to {@link SpiceRequest}s sent from this
         * {@link SpiceManager} will be unregistered. None of them will be
         * notified with the results of their {@link SpiceRequest}s. Unbinding
        ...
        /**
         * This is mostly a testing method. Stops the {@link SpiceManager}. It
         * will unbind from {@link SpiceService}. All request listeners that had
         * been registered to listen to {@link SpiceRequest}s sent from this
         * {@link SpiceManager} will be unregistered. None of them will be
         * notified with the results of their {@link SpiceRequest}s. Unbinding
         * will occur syncrhonously : the method returns when all events have
         * been unregistered and when main processing thread stops.
         */
        public synchronized void shouldStopAndJoin(final long timeOut)
        ...
        /**
         * Get some data previously saved in cache with key
         * <i>requestCacheKey</i> with maximum time in cache :
         * <i>cacheDuration</i> millisecond and register listeners to notify
         * when request is finished. This method executes a SpiceRequest with no
         * network processing. It just checks whatever is in the cache and
         * return it, including null if there is no such data found in cache.
         * @param clazz
         *                the class of the result to retrieve from cache.
         * @param requestCacheKey
         *                the key used to store and retrieve the result of the
         ...
         *                If nothing is found in cache, listeners will receive a
         *                null result on their
         *                {@link RequestListener#onRequestSuccess(Object)}
         *                method. If something is found in cache, they will
         *                receive it in this method. If an error occurs, they
         *                will be notified via their
         ...
         * exists, this method does nothing. If a request identified by clazz
         * and requestCacheKey, it will receive an additional listener.
         * @param clazz
         *                the class of the result of the pending request to look
         *                for.
         * @param requestCacheKey
         *                the key used to store and retrieve the result of the
         *                request in the cache
         ...
         *                the request to execute. {@link CachedSpiceRequest} is
         *                a wrapper of {@link SpiceRequest} that contains cache
         *                key and cache duration
         * @param requestListener
         *                the listener to notify when the request will finish
         */
        ...
        /**
         * Cancel a pending request if it exists. If no such request exists,
         * this method does nothing. If a request identified by clazz and
         * requestCacheKey exisits, it will be cancelled and its associated
         ...
         */
        public <T> void cancel(final Class<T> clazz,
                        final String requestCacheKey) {
                final SpiceRequest<T> request = new SpiceRequest<T>(clazz) {
         ...
         * {@link SpiceService} if the request has already been sent to the
         * service. Otherwise, it will just remove listeners before passing the
         * request to the {@link SpiceService}. Calling this method doesn't
         * prevent request from beeing executed (and put in cache) but will
         * remove request's listeners notification.
         * @param request
        ...
        /**
         * Internal method to remove requests. If request has not been passed to
         * the {@link SpiceService} yet, all listeners are unregistered locally
         * before beeing passed to the service. Otherwise, it will
         * asynchronously ask to the {@link SpiceService} to remove the
         * listeners of the request beeing processed.
         * @param request
         *                Request for which listeners are to unregistered.
         */
        protected void dontNotifyRequestListenersForRequestInternal(
                ...
                try {
                        ...
                        Ln.v("Removed from requests to launch list : "
                                        + requestNotPassedToServiceYet);
        ...
        private boolean removeListenersOfCachedRequestToLaunch(
                ...
                synchronized (mapRequestToLaunchToRequestListener) {
                        for (final CachedSpiceRequest<?> cachedSpiceRequest : mapRequestToLaunchToRequestListener
                                        .keySet()) {
                                if (match(cachedSpiceRequest, request)) {
                                        final Set<RequestListener<?>> setRequestListeners = mapRequestToLaunchToRequestListener
                                                        .get(cachedSpiceRequest);
        ...
        /**
         * Remove all listeners of a request that may have already been passed
         * to the {@link SpiceService}. If the request has already been passed
         * to the {@link SpiceService}, the method will bind to the service and
         * ask it to remove listeners.
         * @param request
         *                the request for which listeners must be unregistered.
         */
        private void removeListenersOfPendingCachedRequest(
                        final SpiceRequest<?> request)
                        throws InterruptedException {
                synchronized (mapPendingRequestToRequestListener) {
                        for (final CachedSpiceRequest<?> cachedSpiceRequest : mapPendingRequestToRequestListener
                                        .keySet()) {
                                if (match(cachedSpiceRequest, request)) {
                                        waitForServiceToBeBound();
                                        ...
                                        final Set<RequestListener<?>> setRequestListeners = mapPendingRequestToRequestListener
                                                        .get(cachedSpiceRequest);
                                        spiceService.dontNotifyRequestListenersForRequest(
                                                        cachedSpiceRequest,
                                                        setRequestListeners);
                                        mapPendingRequestToRequestListener
        ...
        /**
         * Asynchronously ask service to remove all listeners of pending
         * requests.
         * @throws InterruptedException
         *                 in case service binding fails.
         */
        private void removeListenersOfAllPendingCachedRequests()
                        throws InterruptedException {
                synchronized (mapPendingRequestToRequestListener) {
                        if (!mapPendingRequestToRequestListener.isEmpty()) {
                                ...
                                for (final CachedSpiceRequest<?> cachedSpiceRequest : mapPendingRequestToRequestListener
                                        ...
                                        if (setRequestListeners != null) {
                                                Ln.d("Removing listeners of request : "
                                                                + cachedSpiceRequest
                                                                                .toString()
                                                                + " : "
                                                                + setRequestListeners
         ...
         */
        private boolean match(final CachedSpiceRequest<?> cachedSpiceRequest,
                        final SpiceRequest<?> spiceRequest) {
        ...
        private void cancelAllRequestsInternal() {
                try {
                        ...
                        final List<CachedSpiceRequest<?>> listDuplicate = new ArrayList<CachedSpiceRequest<?>>(
                                        mapPendingRequestToRequestListener
                                                        .keySet());
        ...
        public <T> Future<List<T>> getAllDataFromCache(final Class<T> clazz)
        ...
        public <T> Future<T> getDataFromCache(final Class<T> clazz,
                ...
                return executorService.submit(new Callable<T>() {
                        ...
                        public T call() throws Exception {
                                ...
                                return spiceService.getDataFromCache(clazz,
                                                cacheKey);
                        }
                });
        }
                ...
                executorService.execute(new Runnable() {
                        ...
                        public void run() {
                                try {
                                        ...
                                        spiceService.removeDataFromCache(clazz,
                                                        cacheKey);
                                } catch (final InterruptedException e) {
                                        Ln.e(e,
                                                        "Interrupted while waiting for acquiring service.");
         ...
         * Specify wether an error on reading/writing cache must fail the
         * process.
        ...
        private <T> void addRequestListenerToListOfRequestListeners(
                        ...
                        final RequestListener<T> requestListener) {
                synchronized (mapRequestToLaunchToRequestListener) {
                        Set<RequestListener<?>> listeners = mapRequestToLaunchToRequestListener
                                        .get(cachedSpiceRequest);
                        if (listeners == null) {
        ...
        public class SpiceServiceConnection implements ServiceConnection {
                ...
                public void onServiceConnected(final ComponentName name,
                        ...
                        try {
                                ...
                                spiceService = ((SpiceServiceBinder) service)
                                                .getSpiceService();
                                spiceService.addSpiceServiceListener(new RequestRemoverSpiceServiceListener());
                                Ln.d("Bound to service : "
                                                + spiceService.getClass()
                                                                .getSimpleName());
         ...
         */
        private class RequestRemoverSpiceServiceListener implements
                        SpiceServiceServiceListener {
                @Override
                public void onRequestProcessed(
                                final CachedSpiceRequest<?> cachedSpiceRequest) {
                        synchronized (mapPendingRequestToRequestListener) {
                                mapPendingRequestToRequestListener
                                                .remove(cachedSpiceRequest);
                        }
        ...
        private void checkServiceIsProperlyDeclaredInAndroidManifest(
                        final Context context) {
                final Intent intentCheck = new Intent(context,
                                spiceServiceClass);
                if (context.getPackageManager()
                                .queryIntentServices(intentCheck, 0).isEmpty()) {
                        shouldStop();
        ...
        private void dumpMap(
                        ...
                        final Map<CachedSpiceRequest<?>, Set<RequestListener<?>>> map) {
                synchronized (map) {
                        stringBuilder.append(" request count= ");
                        stringBuilder.append(mapRequestToLaunchToRequestListener
                                        .keySet().size());

                        stringBuilder.append(", listeners per requests = [");
                        for (final Map.Entry<CachedSpiceRequest<?>, Set<RequestListener<?>>> entry : map
                                        .entrySet()) {
                                stringBuilder.append(entry.getKey().getClass()
                                                .getName());
                                stringBuilder.append(":");
