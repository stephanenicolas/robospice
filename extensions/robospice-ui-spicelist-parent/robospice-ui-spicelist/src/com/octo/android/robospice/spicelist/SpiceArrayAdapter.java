...
/**
 * An adapter that is optimized for {@link SpiceListView} instances. It offers to update ImageViews contained in
 * {@link SpiceListItemView} instances with images loaded from the network.
 * 
...
public abstract class SpiceArrayAdapter< T > extends ArrayAdapter< T > {
    ...
    /**
     * Updates a {@link SpiceListItemView} containing some data. The method {@link #createRequest(Object)} will be
     * applied to data to know which request to execute to get data from network if needed. This method must be called
     * during {@link #getView(int, android.view.View, android.view.ViewGroup)}.
     * 
     * @param data
    ...
    /**
     * Returns the {@link SpiceRequest} used to update the image associated to a given data.
     * 
     * @param data
     *            the data whose image will be fetched from network by this query.
     * @return the {@link SpiceRequest} used to update the image associated to a given data.
     */
    ...
    protected void loadBitmapAsynchronously( T octo, ImageView thumbImageView, String tempThumbnailImageFileName ) {
        ...
        if ( cancelPotentialWork( tempThumbnailImageFileName, thumbImageView ) ) {
            ...
            int width = Math.round( convertDpToPixel( 45, getContext() ) );
            int height = width;
            task.execute( tempThumbnailImageFileName, String.valueOf( width ), String.valueOf( height ) );
        }
    }
        ...
        protected Boolean doInBackground( Object... params ) {
            ...
            if ( !tempThumbnailImageFile.exists() ) {
                if ( isNetworkFetchingAllowed ) {
                    OctoImageRequestListener octoImageRequestListener = new OctoImageRequestListener( octo, spiceListItemView, tempThumbnailImageFileName );
                    spiceManagerBinary.execute( request, "THUMB_IMAGE_" + octo.hashCode(), DurationInMillis.NEVER, octoImageRequestListener );
                }
                return false;
            }
            return true;
    ...
    private class NetworkFetchingAuthorizationStateChangeAdapter {
        ...
        public void onNetworkFetchingAllowedChange( boolean networkFetchingAllowed ) {
            ...
            if ( spiceListItemView != null ) {
                ...
                new ThumbnailAsynTask( createRequest( data ) ).execute( data, spiceListItemView );
            }
        }
