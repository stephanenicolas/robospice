...
public class RoboSpiceDatabaseHelper extends OrmLiteSqliteOpenHelper {
    ...
    public RoboSpiceDatabaseHelper(Application application,
        String databaseName, int databaseVersion) {
        super(application, databaseName, null, databaseVersion);
    }

    public RoboSpiceDatabaseHelper(Context context, String databaseName,
    ...
    public <T> void updateObjectInDatabase(T modelObject,
        Class<T> modelObjectClass) throws SQLException {
        if (modelObject != null) {
    ...
    public <T> void refreshFromDatabase(T modelObject, Class<T> modelObjectClass)
        throws SQLException {
        if (modelObject != null) {
            Dao<T, ?> dao = getDao(modelObjectClass);
    ...
    public <T, ID> T queryForIdFromDatabase(ID id, Class<T> modelObjectClass)
        throws SQLException {
    ...
    public <T, ID> void deleteByIdFromDataBase(ID id, Class<T> modelObjectClass)
        throws SQLException {
        Dao<T, ID> dao = getDao(modelObjectClass);
        dao.deleteById(id);
    }
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (annotation != null) {
                ParameterizedType stringListType = (ParameterizedType) field
                    .getGenericType();
                Class<?> itemInListClass = (Class<?>) stringListType
    ...
    public <T> void deleteFromDataBase(T modelObject, Class<T> modelObjectClass)
        throws SQLException {
        if (modelObject != null) {
            Dao<T, ?> dao = getDao(modelObjectClass);
    ...
    public <T> void deleteFromDataBase(
        ForeignCollection<T> modelObjectCollection, Class<T> modelObjectClass)
        throws SQLException {
    ...
    public <T, FT> ForeignCollection<FT> getNewEmptyForeignCollection(
        String foreignKeyColumnName, Class<T> modelObjectClass,
        Class<FT> foreignObjectClass) throws SQLException {
        Dao<T, ?> dao = getDao(modelObjectClass);
