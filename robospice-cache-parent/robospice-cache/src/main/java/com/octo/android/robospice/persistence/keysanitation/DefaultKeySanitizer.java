...
public class DefaultKeySanitizer implements KeySanitizer {
    ...
    public Object sanitizeKey(Object cacheKey) {
        if (!(cacheKey instanceof String)) {
            throw new RuntimeException(
                    DefaultKeySanitizer.class.getSimpleName()
                            + " can only be used with Strings cache keys.");
        }
        return Base64.encodeToString(
                ((String) cacheKey).getBytes(UTF8_CHARSET), BASE64_FLAGS);
    }
    ...
    public Object desanitizeKey(Object sanitzedCacheKey) {
        ...
        return new String(
                Base64.decode((String) sanitzedCacheKey, BASE64_FLAGS),
                UTF8_CHARSET);
    }
}
